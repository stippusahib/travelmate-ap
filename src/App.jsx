...
// Contains all CSS for a self-contained, fully styled application.
const GlobalStyles = () => {
  useEffect(() => {
    const css = `
      :root {
      ...
      body.dark-mode {
          ...
          --background-gradient-end: #252D3A;
          --text-color: #E2E8F0;
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .theme-switcher { position: absolute; top: 32px; right: 32px; background: rgba(0, 0, 0, 0.1); backdrop-filter: blur(5px); border: 1px solid rgba(0, 0, 0, 0.1); color: var(--text-color); width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s ease; z-index: 100; }
      ...
      body.dark-mode .search-button { background-image: linear-gradient(to right, #3B82F6, #60A5FA, #93C5FD); }
      ...
      .result-card, .multi-leg-card { background: var(--card-background); border-radius: 16px; box-shadow: 0 4px 16px var(--card-shadow); border: 1px solid var(--border-color); transition: all 0.3s ease; animation: fadeInUp 0.5s ease backwards; }
      ...
      body.dark-mode .provider-logo { background-color: #fff; padding: 5px; }

      .trip-details { text-align: left; }
      .trip-details .mode { font-weight: 600; font-size: 1.05rem; }
      ...
      .multi-leg-header { padding: 16px 20px; border-bottom: 1px solid var(--border-color); }
...
const SearchForm = ({ onSearch, allLocations, onSort, currentSort, onToggleEco, isEco, showFilters }) => {
    ...
    useEffect(() => { setJourneyDate(today); }, [today]);
    return (
        <section className="form-section">
            <form onSubmit={handleSubmit}>
                <div className="search-form-grid">
                    <AutocompleteInput label="From" value={from} onChange={setFrom} placeholder="Enter city" suggestions={fromSuggestions} />
                    <button type="button" className="swap-button" onClick={handleSwap} title="Swap locations">&#8644;</button>
                    <AutocompleteInput label="To" value={to} onChange={setTo} placeholder="Enter city" suggestions={toSuggestions} />
                </div>
                <div className="date-picker-grid">
                     <div className="input-group"> <label>Journey Date</label> <input type="date" value={journeyDate} min={today} onChange={e => setJourneyDate(e.target.value)} className="search-input" /> </div>
                </div>
...
function App() {
    ...
    const allLocations = useMemo(() => { const locations = new Set(); travelData.forEach(route => { locations.add(route.from); locations.add(route.to); if(route.type === 'connected') { route.legs.forEach(leg => { locations.add(leg.from); locations.add(leg.to); }); } }); return Array.from(locations); }, []);
    const handleSearch = (from, to) => { const fromQuery = from.toLowerCase().trim(); const toQuery = to.toLowerCase().trim(); const foundRoute = travelData.find(r => r.from.toLowerCase() === fromQuery && r.to.toLowerCase() === toQuery); setResults(foundRoute || { type: 'none' }); setSortBy(null); setEcoFriendlyOnly(false); };
